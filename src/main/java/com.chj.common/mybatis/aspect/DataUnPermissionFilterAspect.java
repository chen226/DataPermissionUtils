...
@Order(3)
@Component
public class DataUnPermissionFilterAspect {

    @Around("@annotation(com.chj.common.mybatis.annotation.UnDataPermissionTable)")
    public Object DataUnPermissionFilter(ProceedingJoinPoint point) throws Throwable {
        Class<?> className = point.getTarget().getClass();
        String[] tableNames = {};
        if (className.isAnnotationPresent(UnDataPermissionTable.class)) {
            UnDataPermissionTable ds = className.getAnnotation(UnDataPermissionTable.class);
            ...
            if (method.isAnnotationPresent(UnDataPermissionTable.class)) {
                UnDataPermissionTable annotation = method.getAnnotation(UnDataPermissionTable.class);
                // 取出注解中所需要做数据权限的表
